...
(function () {
  ...
  const SECTIONS = [
    ...
    { title: 'Thursday classes', controls: [
      ...
      { k: 'class_card_size', label: 'Class card pack size (L2 & social-only)', type: 'int', min: 1, max: 50, help: 'If more than 1, Level 2 and social-only tickets are sold in packs of this size (a "class card") bought in one go and usable on any night, so the Dancecloud fee is charged once per pack instead of per ticket. 1 = pay per night. This is not an L1 term block: the tickets are not pinned to dates.' },
    ...
    { title: 'Weekender', controls: [
      ...
      { k: 'weekender_room_per_hour', label: 'Room hire per hour', type: 'money', help: 'Hourly hire rate for the weekender rooms.' },
    ...
    { title: 'Volunteers, committee & accounts', controls: [
      ...
      { k: 'n_safer_spaces', label: 'Safer-spaces team', type: 'int', min: 0, max: 100, help: 'Safer-spaces volunteers, also at the volunteer socials. Count only those not already counted as committee members; anyone on both teams is counted once, under committee.' },
      { k: 'safer_spaces_accounts', label: 'Safer-spaces team have Workspace accounts', type: 'toggle', help: 'If on, each safer-spaces volunteer holds a paid Google Workspace seat. Turn off if they no longer get accounts, and they drop out of the Workspace bill (they still attend the volunteer socials).' },
